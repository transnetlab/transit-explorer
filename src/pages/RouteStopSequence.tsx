...
import { ArrowLeft, SplitSquareHorizontal, Info, AlertCircle, Loader2, CheckCircle } from 'lucide-react';
import { copyGtfsFiles, generateRouteStopSequences } from '../api';

export function RouteStopSequence() {
  ...
  const doneKey = useMemo(() => {
    if (isDefaultCity) {
      const c = (city || '').trim();
      return c ? `preprocessing:route_stop_sequence:done:default:${c}` : '';
    }
    const id = payload.unique_city_id;
    return id ? `preprocessing:route_stop_sequence:done:${id}` : '';
  }, [city, isDefaultCity, payload.unique_city_id]);

  const canRun = !!payload.user_id && !!payload.api_key && !!payload.unique_city_id;

  useEffect(() => {
    // Match the UX of clicking the "Sequence routes" button in other pages:
    // opening this page should immediately prompt for confirmation.
    setShowPrompt(true);
  }, []);
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await generateRouteStopSequences(payload);
      if (res && typeof res === 'object' && 'success' in res && (res as any).success === false) {
        throw new Error((res as any)?.message || 'Route stop sequence generation failed');
      }
      clearInterval(timer);
      setProgress(100);
      setDoneMessage(res?.message || 'Route stop sequences generated successfully.');
      if (doneKey) {
        try {
      ...
      setError(e?.message || 'Failed to generate route stop sequences');
    } finally {
      setRunning(false);
    }
  };
      ...
      <div className="container mx-auto px-4 py-10 max-w-3xl">
        <div className="flex items-start justify-between gap-4 mb-6">
          ...
          <div className="flex-1">
            <h1 className="text-2xl font-bold text-blue-600 dark:text-blue-400">Route Stop Sequence</h1>
        ...
        {showInfo && (
          <div className="mb-6 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4 text-sm text-blue-800 dark:text-blue-200">
            ...
            <p>
              Creates new route variants from unique stop sequences (unique segments) found in the trips. For example, if a route has
              trips using stops 1 → 3 → 5 → 4 and the same route also has trips using stops 1 → 3 → 4, it creates two new routes so
              each distinct stop pattern is represented separately.
            </p>
          </div>
        )}
        ...
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-6 shadow-sm">
          <div className="flex items-center justify-between gap-4">
            <div>
              <h2 className="text-base font-semibold">Generate sequences</h2>
              <p className="text-xs text-gray-600 dark:text-gray-300 mt-1">
                For default cities (Bangalore/Paris/Austin/Sydney/Colombia), this uses built-in credentials. Otherwise requires `user_id`, `api_key`,
                and `unique_city_id` in localStorage.
              </p>
            </div>
            ...
            >
              ...
              {running ? 'Running…' : 'Sequence routes'}
            </button>
          </div>
          ...
          {running && (
            <div className="mt-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <p className="text-xs text-gray-700 dark:text-gray-300">Generating…</p>
              </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl p-6 w-full max-w-md">
              ...
              <p className="text-gray-600 dark:text-gray-300 mb-4 text-sm">
                This preprocessing generates derived routes based on unique stop sequences.
              </p>
              {!canRun && (
