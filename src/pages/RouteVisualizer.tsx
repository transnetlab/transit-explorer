...
export function RouteVisualizer() {
  ...
  const handleRouteSelect = async (routeId: string) => {
    ...
    try {
      console.log(`Fetching shortest path for route ${routeId}`);
      const path = await getShortestPath(routeId, city);
      console.log(`Received path with ${path.length} points`);
  ...
  <div className="h-full p-4 sm:p-6 overflow-y-auto">
          <div className="pt-12 md:pt-0">
            <div className="flex items-center justify-between mb-4">
              ...
              <h1 className="text-2xl font-bold text-blue-600 dark:text-blue-400">Route Finder</h1>
              <div />
            </div>
      ...
      `}>
        ...
        <Map path={selectedPath} selectedRouteId={selectedRouteId} center={cityCenter} />
      </div>
    </div>
  );
}